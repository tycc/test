(function()
{
	...
	iScroll=function(el,options)
	{
		...
		if(that.options.checkDOMChanges)
			that.checkDOMTime=setInterval(function(){that._checkDOMChanges();},500);
	};
	iScroll.prototype=
	{
		enabled:true,
		...
		_scrollbar:function(dir)
		{
			...
			if(!that[dir+'ScrollbarWrapper'])
			{
				...
				that[dir+'ScrollbarWrapper'].appendChild(bar);that[dir+'ScrollbarIndicator']=bar;
			}
			if(dir=='h')
			{
		...
		_scrollbarPos:function(dir,hidden)
		{
			...
			if(pos<0)
			{
				if(!that.options.fixedScrollbar)
				{
					...
					if(size<8)
						size=8;that[dir+'ScrollbarIndicator'].style[dir=='h'?'width':'height']=size+'px';
				}
				pos=0;
			}
		...
		_move:function(e)
		{
			...
			that.moved=true;
			that._pos(newX,newY);
			that.dirX=deltaX>0?-1:deltaX<0?1:0;that.dirY=deltaY>0?-1:deltaY<0?1:0;
		...
		_end:function(e)
		{
			...
			if(that.zoomed)
			{
				...
				that.lastScale=scale/that.scale;that.scale=scale;
		...
		destroy:function()
		{
			var that=this;that.scroller.style[vendor+'Transform']='';
			that.hScrollbar=false;that.vScrollbar=false;that._scrollbar('h');
			that._scrollbar('v');
		...
		refresh:function()
		{
			...
			that.hScrollbar=that.hScroll&&that.options.hScrollbar;that.vScrollbar=that.vScroll&&that.options.vScrollbar&&that.scrollerH>that.wrapperH;
			...
			if(typeof that.options.snap=='string')
			{
				...
				for(i=0,l=els.length;i<l;i++)
				{
					...
					that.pagesX[i]=pos.left<that.maxScrollX?that.maxScrollX:pos.left*that.scale;that.pagesY[i]=pos.top<that.maxScrollY?that.maxScrollY:pos.top*that.scale;
				}
			}
		...
		zoom:function(x,y,scale,time)
		{
			...
			if(!that.options.useTransform)
				return;that.zoomed=true;
			time=time===undefined?200:time;
			x=x-that.wrapperOffsetLeft-that.x;
			y=y-that.wrapperOffsetTop-that.y;
			that.x=x-x*relScale+that.x;that.y=y-y*relScale+that.y;
			that.scale=scale;that.refresh();
			that.x=that.x>0?0:that.x<that.maxScrollX?that.maxScrollX:that.x;that.y=that.y>that.minScrollY?that.minScrollY:that.y<that.maxScrollY?that.maxScrollY:that.y;
